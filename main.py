...
class Restaurant():
    ...
    def newOrder(self):
        if self.active_user[1][2] == "Admin" or self.active_user[1][2] == 'User':
            ...
            if resp2.lower() == "y":
                ...
                print("Please pay {0}".format(sum1))
                confirm = input("Please type 'CONFIRM' to pay ₹{} : ".format(sum1))
                if confirm == 'CONFIRM':
                    print("Order Successful. Enjoy!")
                    for l in range(len(present_order)):
                        self.orderHistory.append([self.active_user[1][0], present_order[l][0], present_order[l][-1]])
                else:
    ...
    def updateProfile(self):
        if self.active_user[1][2] == "Admin" or self.active_user[1][2] == 'User':
            userid = input("Please enter your email id: ")
            for i in range(len(self.users)):
                if userid in self.users[i]:
                    print(" ")
                    print("Welcome {0}".format(self.users[i][0]))
                    print(" ")
                    print("Press 1: Edit Name: ")
                    print("Press 2: Edit Contact no.: ")
                    print("Press 3: Edit Address: ")
                    print("Press 4: Edit Password: ")
                    input12 = input("Please enter your response: ")
                    if input12 == '1':
                        self.users[i][0] = input("Please enter new name: ")
                        print(" ")
                        print("Changes Applied!")
                    elif input12 == '2':
                        self.users[i][3] = input("Please enter new Contact no.: ")
                        print(" ")
                        print("Changes Applied!")
                    elif input12 == '3':
                        self.users[i][4] = input("Please enter new Address: ")
                        print(" ")
                        print("Changes Applied!")
                    elif input12 == '4':
                        self.users[i][2] = input("Please enter new Password: ")
                        print(" ")
                        print("Changes Applied!")
                    break
            else:
    ...
    def orderhistory1(self):
        if self.active_user[1][2] == "Admin" or self.active_user[1][2] == "User":
            new_list3 = []
            for i in range(len(self.orderHistory)):
                if self.active_user[1][0] in self.orderHistory[i][0]:
                    new_list3.append(self.orderHistory[i])
            print(tabulate(new_list3, headers=self.orderHistory[0], tablefmt='fancy_grid'))
        else:
            print("Please login first!")
